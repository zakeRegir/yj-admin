    ...
    modelComputed: {
      get() {
        ...
        if (this.column.type === 'currency') {
          return this.isForce
            ? val
            : currency(val, this.column.currency, this.column.decimals)
        }
        return val
      },
      set(value) {
        ...
        try {
          if (
            this.getStrFunction(`this.row.${this.computedColumn.prop}`) ===
            undefined
          ) {
            this.setRowKey(value)
          } else {
            this.getStrFunction(`this.row.${this.computedColumn.prop} = value`)
          }
        } catch (error) {
          this.setRowKey(value)
        }
    ...
    getParams() {
      ...
      if (
        ...
        for (const key in this.computedColumn.params) {
          ...
          newObj[key] =
            value !== undefined ? value : this.computedColumn.params[key]
        }
      }
      return newObj
    }
        ...
        return h(
          componentType,
          {
            props: {
              ...
              placeholder,
              params:
                componentType === 'm-select' ? getParams(computedColumn) : null,
              value: modelComputed,
              customRender:
                computedColumn.slots && computedColumn.slots.default
        ...
        return h(
          componentType,
          {
            props: {
              ...
              label:
                componentType === 'el-checkbox' || componentType === 'el-radio'
                  ? null
                  : computedColumn.label
            },
            attrs: attrs,
